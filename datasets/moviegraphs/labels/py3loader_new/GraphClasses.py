...
class MovieGraph(object):
    ...
    def visualize_all_graphs(self, fname='test.pdf', prop_labels=False):
        ...
        ----------
        fname : str, optional
            Output PDF filename (default='test.pdf')
        prop_labels : bool, optional
            Whether to show property labels (default=False)
        """
        
        with PdfPages(fname) as write_pdf:
            for sid, clip_graph in self.clip_graphs.items():
                try:
                    plt.figure(figsize=(8, 5))
                    clip_graph.visualize_graph(str(sid), prop_labels=prop_labels)
                    write_pdf.savefig(bbox_inches='tight', dpi=150)
                    plt.close()
                except Exception as e:
                    print(f"Cannot plot for {sid}: {e}")
